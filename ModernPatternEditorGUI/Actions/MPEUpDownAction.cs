        ...
        private void ShiftValue(int shiftdelta)
        {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    if (newValue >= 0)
                    {
                        if (digitIterator.ParameterColumn.PatternColumn.Parameter.Type == ParameterType.Note && newValue != BuzzNote.Off)
                        {
                            if (newValue + shiftdelta > BuzzNote.Min && newValue + shiftdelta < BuzzNote.Max)
                            {
